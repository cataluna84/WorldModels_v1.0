...
def master():
    ...
    while True:
        ...
        h = (
            t, curr_time, avg_reward, r_min, r_max, std_reward, int(es.rms_stdev() * 100000) / 100000.,
            mean_time_step + 1.,
            int(max_time_step) + 1)
        ...
        with open(filename, 'wt') as out:
            json.dump([np.array(es.current_param()).round(4).tolist()], out, sort_keys=True, indent=2,
                      separators=(',', ': '))

        with open(filename_hist, 'wt') as out:
            json.dump(history, out, sort_keys=False, indent=0, separators=(',', ':'))

        pickle.dump(es, open(filename_es + str(t), 'wb'))
        ...
        if (t % eval_steps == 0):  # evaluate on actual task at hand
            ...
            with open(filename_best + str(t), 'wt') as out:
...
def main(args):
    global env_name, optimizer, init_opt, num_episode, eval_steps, max_length, num_worker, num_worker_trial, \
        antithetic, seed_start, retrain_mode, dream_mode, cap_time_mode  # , vae_version, rnn_version,
...
def mpi_fork(n):
    ...
    if os.getenv("IN_MPI") is None:
        ...
        # TODO: Fix this bug - Initial analysis suggests that during the spawn of the parent process from the
        #  fork of mpi, due to the combination of time limit of CPU to spawn up new processes and the CPU cores
        #  being overworked (too).
        #  System logs gave the following info:
        #  Time: 10:24:41
        #  Message: kernel: Out of memory: Killed process 15320 (python) total-vm:12555940kB, anon-rss:1570012kB, file-rss:76504kB, shmem-rss:14672kB
        #  Priority: 3
        #  Possible Solution: Try reducing the total number of threads to 7, leaving a core for other management tasks

        '''
        ===================================================================================
        =   BAD TERMINATION OF ONE OF YOUR APPLICATION PROCESSES
        =   PID 15320 RUNNING AT home
        =   EXIT CODE: 9
        ...
        ['mpirun', '-np', '9', '/home/cataluna84/anaconda3/envs/worldmodels/bin/python', '05_train_controller.py', 'car_racing', '-n', '8', '-t', '2', '-e', '4', '--max_length=1000', '--init_opt', './controller/train_2/car_racing.cma.4.16.es.pk125']
        Traceback (most recent call last):
          File "05_train_controller.py", line 538, in <module>
            if "parent" == mpi_fork(args.num_worker + 1): os.exit()
          File "05_train_controller.py", line 502, in mpi_fork
            subprocess.check_call(["mpirun", "-np", str(n), sys.executable] + ['-u'] + sys.argv, env=env)
          File "/home/cataluna84/anaconda3/envs/worldmodels/lib/python3.6/subprocess.py", line 311, in check_call
            raise CalledProcessError(retcode, cmd)
        subprocess.CalledProcessError: Command '['mpirun', '-np', '9', '/home/cataluna84/anaconda3/envs/worldmodels/bin/python', '-u', '05_train_controller.py', 'car_racing', '-n', '8', '-t', '2', '-e', '4', '--max_length=1000', '--init_opt', './controller/train_2/car_racing.cma.4.16.es.pk125']' returned non-zero exit status 9.
        '''
        '''
